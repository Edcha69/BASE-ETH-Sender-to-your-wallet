...
# Настройки сети Base
BASE_RPC_URL = "https://mainnet.base.org"  # Публичный RPC-узел для Base
...
# Подключение к Base
def connect_to_base(rpc_url, proxy=None):
    try:
        if proxy:
        ...
        print(f"Ошибка подключения к Base RPC: {e}")
        return None
...
def send_eth(w3, private_key, from_address, to_address, amount_wei):
    try:
        ...
        tx = {
            ...
            'chainId': 8453  # Chain ID для Base mainnet
        }
...
def main():
    ...
    for i, (private_key, to_address) in enumerate(zip(private_keys, to_addresses)):
        ...
        w3 = connect_to_base(BASE_RPC_URL, proxy)
        
        if not w3 or not w3.is_connected():
            print(f"Ошибка: не удалось подключиться к Base RPC через прокси {proxy}")
            continue
